...
async function identifyLocal(file){
  ...
  if(ext==='pdf'){
    const fd=new FormData();fd.append('file',file);fd.append('master_json',JSON.stringify(masterRows));
    try{
    ...
    }catch(e){return {customer:'',hq:'',confidence:0,match_status:'IDENTIFICATION ERROR',source:'API error',error:e.message,stockists:[]}}
  }
  return {customer:'',hq:'',confidence:0,match_status:'NON-PDF',source:'Manual selection',stockists:[]};
}
async function validateFiles(){
  if(!masterLoaded||!files.length)return;
  progress(3,'Reading PDF headers','Reading HQ and Stockist from each statement…');
  ...
  for(let i=0;i<files.length;i++){
    ...
    if(x.match_status==='EXACT'||x.match_status==='HIGH CONFIDENCE'){
      $('st-'+i).innerHTML='<span class="pill ok">✓ PDF identity matched</span>';
    }else if(x.hq){
    ...
    done++;progress(5+done/files.length*20,'PDF identity validation',`${done}/${files.length} statements checked`);await new Promise(r=>setTimeout(r,0));
  }
  updateCount();
}
...
function poolFor(customer,hq){return masterRows.filter(r=>r.HQ===hq&&r['CUSTOMER NAME']===customer).map(r=>({HQ:r.HQ,CUSTOMER_NAME:r['CUSTOMER NAME'],SKU_NAME:r['SKU NAME'],PTS:r.PTS}))}
async function analyzeFile(i,aliasesForRun){const f=files[i],customer=$('cust-'+i).value,hq=$('hq-'+i).value,pool=poolFor(customer,hq);const fd=()=>{const x=new FormData();x.append('file',f);x.append('pool_json',JSON.stringify(pool));x.append('aliases_json',JSON.stringify(aliasesForRun||{}));x.append('threshold',$('threshold').value);return x};let last='Unknown API error';for(let attempt=1;attempt<=2;attempt++){const ctl=new AbortController();const timer=setTimeout(()=>ctl.abort(),45000);try{const r=await fetch('/api/analyze',{method:'POST',body:fd(),signal:ctl.signal});clearTimeout(timer);const text=await r.text();if(!r.ok)throw Error(text||`API returned ${r.status}`);const data=JSON.parse(text);return {...data,hq,customer}}catch(e){clearTimeout(timer);last=e.name==='AbortError'?'API timed out after 45 seconds':(e?.message||String(e));if(attempt<2)await new Promise(r=>setTimeout(r,600))}}throw Error(last)}
