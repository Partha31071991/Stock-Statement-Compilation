...
async def analyze_endpoint(request: Request):
    try:
        ...
        threshold = int(request.query_params.get('threshold','82') or 82)
        aliases = {}
        try:
            aliases = json.loads(request.headers.get('x-aliases','{}') or '{}')
        except Exception:
            aliases = {}
        master = load_master_cache()
        pool = [r for r in master if str(r.get('HQ','')) == str(hq) and str(r.get('CUSTOMER NAME','')) == str(customer)]
